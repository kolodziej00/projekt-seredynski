...
class CA:
    ...
    def calculate_statistics(self):
        ...
        for i in range(1, self.M_rows - 1):
            for j in range(1, self.N_cols - 1):
                ...
                if self.cells[i, j].state == 1:
                    num_of_C += 1
                    if self.is_C_correct(i, j):
                        num_of_C_corr += 1

                # all D
                if self.cells[i, j].strategy == 0:
                    num_of_allD += 1
                # all C
                elif self.cells[i, j].strategy == 1:
                    num_of_allC += 1
                # kD
                elif self.cells[i, j].strategy == 2:
                    num_of_kD += 1
                    if self.cells[i, j].k == 0:
                        num_of_0D += 1
                    elif self.cells[i, j].k == 1:
                        num_of_1D += 1
                    elif self.cells[i, j].k == 2:
                        num_of_2D += 1
                    elif self.cells[i, j].k == 3:
                        num_of_3D += 1
                    elif self.cells[i, j].k == 4:
                        num_of_4D += 1
                    elif self.cells[i, j].k == 5:
                        num_of_5D += 1
                    elif self.cells[i, j].k == 6:
                        num_of_6D += 1
                    elif self.cells[i, j].k == 7:
                        num_of_7D += 1
                    elif self.cells[i, j].k == 8:
                        num_of_8D += 1

                # kC
                elif self.cells[i, j].strategy == 3:
                    num_of_kC += 1
                    if self.cells[i, j].k == 0:
                        num_of_0C += 1
                    elif self.cells[i, j].k == 1:
                ...
                # kDC
                elif self.cells[i, j].strategy == 4:
                    num_of_kDC += 1
                    if self.cells[i, j].k == 0:
                        num_of_0DC += 1
                    elif self.cells[i, j].k == 1:
        ...
        statistics = Statistics(iter, f_C, f_C_corr, av_sum, f_allC, f_allD, f_kD, f_kC,
    ...
    def is_C_correct(self, i, j):
        if self.cells[i, j].state == 1:
            if self.cells[i - 1, j - 1].state == 0 and self.cells[i - 1, j].state == 0 and self.cells[i - 1, j + 1].state == 0:
                if self.cells[i, j - 1].state == 0 and self.cells[i, j + 1].state == 0 and self.cells[i + 1, j - 1].state == 0:
                    if self.cells[i + 1, j].state == 0 and self.cells[i + 1, j + 1].state == 0:
                        return True
        return False
